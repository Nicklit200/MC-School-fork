...
public class CardService {
    ...
    public ImportPreviewResponse previewImport(ImportPreviewRequest request) {
        return importParser.parse(request.rawText(), request.questionAnswerSeparator(), request.cardSeparator());
    }

    @Transactional
    public List<CardResponse> importCardsIntoHomework(AuthenticatedUser teacher, UUID homeworkId, ImportCardsRequest request) {
        User teacherEntity = requireTeacher(teacher.id());
        Homework homework = requireOwnedHomework(teacher.id(), homeworkId);
        List<CardResponse> created = request.cards().stream().map(parsed -> {
            Card card = Card.createImported(homework, teacherEntity, parsed.question(), parsed.correctAnswer(),
                    parsed.wrongAnswer1(), parsed.wrongAnswer2(), parsed.wrongAnswer3());
            card.changeTimeLimit(parsed.timeLimitSeconds());
            return cardRepository.save(card);
        }).map(CardResponse::from).toList();
        if (!created.isEmpty()) {
            cardPushNotificationService.notifyCardsAssigned(homework.getStudent().getId(), homework.getId(), homework.getStartDate());
        }
        return created;
    }

    @Transactional
    public CardResponse createCardInHomework(AuthenticatedUser teacher, UUID homeworkId, CreateCardRequest request) {
        User teacherEntity = requireTeacher(teacher.id());
        Homework homework = requireOwnedHomework(teacher.id(), homeworkId);
    ...
    public List<CardResponse> listCards(AuthenticatedUser teacher, UUID studentId) {
        ...
        return cardRepository.findAllByStudentIdAndArchivedFalseOrderByCreatedAtDesc(studentId).stream().map(CardResponse::from).toList();
    }

    @Transactional(readOnly = true)
    public List<CardResponse> listCardsForHomework(AuthenticatedUser teacher, UUID homeworkId) {
        Homework homework = requireOwnedHomework(teacher.id(), homeworkId);
        return cardRepository.findAllByHomeworkIdAndArchivedFalseOrderByCreatedAtDesc(homework.getId()).stream().map(CardResponse::from).toList();
    }
    ...
    @Transactional
    public void deleteCard(AuthenticatedUser teacher, UUID cardId) { requireOwnedCard(teacher.id(), cardId).archive(); }

    private User requireTeacher(UUID teacherId) {
        return userRepository.findById(teacherId).orElseThrow(() -> new ResourceNotFoundException("Teacher account no longer exists"));
    }

    private User requireOwnedStudent(UUID teacherId, UUID studentId) {
        return userRepository.findById(studentId)
                .filter(u -> u.getRole() == Role.STUDENT).filter(u -> !u.isArchived())
    ...
    private Card requireOwnedCard(UUID teacherId, UUID cardId) {
        return cardRepository.findById(cardId).filter(card -> !card.isArchived())
                .filter(card -> card.getStudent().getTeacher() != null && card.getStudent().getTeacher().getId().equals(teacherId))
                .orElseThrow(() -> new ResourceNotFoundException("Card not found"));
    }

    private Homework requireOwnedHomework(UUID teacherId, UUID homeworkId) {
        return homeworkRepository.findById(homeworkId)
                .filter(homework -> homework.getStudent().getTeacher() != null && homework.getStudent().getTeacher().getId().equals(teacherId))
